...
public final class RoomDefinition {
    ...
    public RelationState getState(IAudioSource sender, ISonusPlayer receiver) {
        if (sender.getUniqueId(receiver).equals(receiver.getUniqueId())) {
            return RelationState.HIDE;
        }
        // check for static relation
        RelationState staticState = this.staticOverrides.get(sender.getUniqueId(receiver), receiver.getUniqueId());
        if (staticState != null) {
            return staticState;
        }
